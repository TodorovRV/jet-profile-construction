...
from utils_local import (create_grid, create_mask, mas_to_rad, v_round,
...
def plot(contours=None, colors=None, vectors=None, vectors_values=None, x=None,
         ...
         colorbar_label=None, show=True, contour_color='k', vector_width=0.05,
    ...
    if vectors is not None:
        ...
        vec = ax.quiver(y[::vinc], x[::vinc], u[::vinc, ::vinc],
                        ...
                        width=vector_width, headaxislength=0., color=vector_color)
...
class Jet_data(CleanImage):
    ...
    def from_hdulist(self, hdulist, set_stokes=None, max_pos=None):
        """
        ...
        image_params = get_fits_image_info_from_hdulist(hdulist)
        pr_hdu = get_hdu_from_hdulist(hdulist)
        img = pr_hdu.data.squeeze()
        if max_pos is None:
            max_pos = np.unravel_index(np.argmax(img), img.shape)
        # image_params["pixref"] = (int(max_pos[1]), int(max_pos[0]))
        try:
        ...
        self._image_dict[set_stokes] = img
        self._image_dict[set_stokes][np.isnan(self._image_dict[set_stokes])] = 0.
        
        # self.pixref = np.unravel_index(np.argmax(self._image_dict[set_stokes]), self._image_dict[set_stokes].shape)
        # self._image_dict[set_stokes] = np.roll(self._image_dict[set_stokes], \
        #         (int(self.imsize[1] / 2)-max_pos[1], int(self.imsize[0] / 2)-max_pos[0]))
        # print(int(self.imsize[1] / 2)-max_pos[1], int(self.imsize[0] / 2)-max_pos[0])

    def from_fits(self, fname, set_stokes=None, max_pos=None):
        """
        ...
        """
        hdulist = pf.open(fname)
        self.from_hdulist(hdulist, set_stokes=set_stokes, max_pos=max_pos)
    ...
    def get_image(self, stk=None):
        ...
        if stk == "P":
            ...
            elif "P" in self.stokes:
                return self._image_dict["P"]
            else:
                raise Exception("Load polarisation images!")
        elif stk == "CHI":
            if "Q" in self.stokes and "U" in self.stokes:
                return 0.5*np.arctan2(self.stokes["U"], self.stokes["Q"])
            else:
                raise Exception("Load polarisation images!")
        elif stk == "F":
            if "Q" in self.stokes and "U" in self.stokes and "I" in self.stokes:
        ...
        elif stk in self.stokes:
            return self._image_dict[stk]
        else:
            raise Exception(f"Unknown stokes: {stk}! Avalible: {list(self._image_dict.keys())}")
